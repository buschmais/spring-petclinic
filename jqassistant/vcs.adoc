...
.The branches "violations", "jqa-remote" and "heads/jqa-testimpactanalysis" must include the latest commit (HEAD) of "master", i.e. merges are required from master into these branches.
----
MATCH
  (master:Branch),
  (branch:Branch)
